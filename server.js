...
// (por si la instancia no trae fetch global)
...
const ROOT = __dirname;                    // donde está server.js
const PUBLIC = path.join(__dirname, 'public');

app.use(express.static(ROOT));             // sirve index.html si está al lado de server.js
app.use(express.static(PUBLIC));           // y también todo lo que haya en /public
...
async function tickerBinance(symbol){ const j = await fetchJson(`https://api.binance.com/api/v3/ticker/bookTicker?symbol=${symbol}`); return { bid_usd:+j.bidPrice, ask_usd:+j.askPrice }; }
async function tickerOKX(symbol){ const inst=symbol.replace('USDT','-USDT'); const j=await fetchJson(`https://www.okx.com/api/v5/market/ticker?instId=${inst}`); const d=j.data?.[0]||{}; return { bid_usd:+d.bidPx, ask_usd:+d.askPx }; }
async function tickerBybit(symbol){ const j=await fetchJson(`https://api.bybit.com/v5/market/tickers?category=spot&symbol=${symbol}`); const d=j.result?.list?.[0]||{}; return { bid_usd:+d.bid1Price, ask_usd:+d.ask1Price }; }
async function tickerKucoin(symbol){ const inst=symbol.replace('USDT','-USDT'); const j=await fetchJson(`https://api.kucoin.com/api/v1/market/orderbook/level1?symbol=${inst}`); const d=j.data||{}; return { bid_usd:+d.bestBid, ask_usd:+d.bestAsk }; }
async function tickerBitget(symbol){ const j=await fetchJson(`https://api.bitget.com/api/spot/v1/market/ticker?symbol=${symbol}`); const d=j.data||{}; return { bid_usd:+d.bestBid, ask_usd:+d.bestAsk }; }
async function tickerMexc(symbol){ const j=await fetchJson(`https://api.mexc.com/api/v3/ticker/bookTicker?symbol=${symbol}`); return { bid_usd:+j.bidPrice, ask_usd:+j.askPrice }; }
...
app.get('/api/precios', async (req, res) => {
  try {
    ...
    const rows = results.map((r, i) => {
      ...
      return { exchange, pair: symbol, bid_usd, ask_usd,
...
// ✅ Fallback para servir index.html en '/'
app.get('/', (_req, res) => {
  // intenta raíz; si no está, sirve /public/index.html
  const rootIndex = path.join(ROOT, 'index.html');
  const publicIndex = path.join(PUBLIC, 'index.html');
  res.sendFile(rootIndex, (err) => {
    if (err) res.sendFile(publicIndex, (err2) => {
