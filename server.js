...
import fetch from 'node-fetch'; // ✅ fetch estable
...
async function getUsdArsVal(){
  ...
  usdarsCache = { v: val, t: now }; return val;
}
...
app.get('/api/precios', async (req, res) => {
  try {
    ...
    const rows = results.map((r,i)=>{
      ...
      if (r.status !== 'fulfilled' || !r.value?.bid_usd || !r.value?.ask_usd) return { exchange, pair:symbol, error:true };
